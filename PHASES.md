...
**Overall ≈ 90% for launch scope** (airplane-mode descoped 2026-07-15). The remaining gate is almost entirely **Phase 4 — the real migration with Dan's Mangomint export** — plus a short pre-launch checklist (service-key rotation, a real live-card money check, decide what to do with ~130 imported clients that have no phone/email). The solo code lane is drained; what's left needs Dan + real calendar time.

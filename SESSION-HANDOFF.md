...
### What's left / next
- **Push notifications — code path DONE & deployed (2026-06-24).** Verified the full chain end-to-end: app registers + saves the device token (`save_device_token` RPC, shop_id + ios), and new-booking/reschedule/check-in all call `fireStaffPush` → `/api/push`. Fixed `api/push.js` to try **PRODUCTION APNs first**, sandbox fallback (was backwards — sandbox-first wasted a failed knock on every real push now that the app ships `aps-environment=production`). Committed `2099b42`, deployed to gotvero.com, endpoint verified live (405 GET / 400 missing-fields). **STILL TO CONFIRM (needs Dan):** (a) the 3 Vercel env keys `APNS_KEY`/`APNS_KEY_ID`/`APNS_TEAM_ID` are actually set, and (b) a real booking actually buzzes Dan's/Heather's phone — that live-device test is the only conclusive proof and hasn't been done yet.

...
export async function sendEmail({ to, subject, text, html, from, fromName, replyTo }) {
  // [email-from-name] Show the shop NAME as the sender ("Sanctuary Barber Co <verified@domain>") while
  // sending from our verified domain — a shop's own gmail can't be a verified sender (breaks SPF/DKIM →
  // spam), so we send from our domain and route replies to their address, exactly like Mangomint does.
  const addr = from || process.env.EMAIL_FROM || "onboarding@resend.dev";
  const fromField = (fromName && !/[<>]/.test(addr)) ? `${String(fromName).replace(/[<>"]/g, "")} <${addr}>` : addr;
...
export function renderEmailHtml(template, ctx) {
  ...
  // [email-one-font] Dan hates mixed fonts/sizes ("just confusing"). ONE family (sans) everywhere in
  // the body; the only serif is the logo, which is an image. Tight type scale: 15 (headline/body),
  // 14 (secondary), 13.5/11 (footer). No 19px serif service "heading" anymore.
  ...
  const blocks = {
    "{appointment}": () => card(
      `<div style="font-size:15px;color:${INK};font-weight:700">${esc(c.service)}</div>` +
      (addonsStr ? `<div style="color:${SUB};font-size:14px;margin-top:3px">(${esc(addonsStr)})</div>` : "") +
      `<div style="color:${SUB};font-size:14px;margin-top:9px">with <b style="color:${INK};font-weight:600">${esc(c.provider)}</b> at <b style="color:${INK};font-weight:600">${esc(c.time)}</b></div>` +
      (c.date ? `<div style="color:${SUB};font-size:14px;margin-top:2px">on <b style="color:${INK};font-weight:600">${esc(c.date)}</b></div>` : "")
    ),
    // waitlist-JOINED confirmation: service + add-ons + barber, then the requested day/time windows (c.waitDates)
    "{waitlist request}": () => card(
      `<div style="font-size:15px;color:${INK};font-weight:700">${esc(c.service)}</div>` +
      (addonsStr ? `<div style="color:${SUB};font-size:14px;margin-top:3px">(${esc(addonsStr)})</div>` : "") +
      `<div style="color:${SUB};font-size:14px;margin-top:9px">with <b style="color:${INK};font-weight:600">${esc(c.provider)}</b></div>` +
      (c.waitDates ? `<div style="color:${INK};font-weight:600;font-size:14px;margin-top:13px">Requested date(s):</div><div style="color:${SUB};font-size:14px;margin-top:3px;line-height:1.6">${esc(c.waitDates).replace(/\n/g, "<br>")}</div>` : "")
    ),
    "{location}": () => card(
      `<div style="font-size:15px;color:${INK};margin-bottom:4px;font-weight:700">${esc(c.locName || c.business)}</div><div style="color:${SUB};font-size:14px">${esc(c.address || "")}</div>`
    ),
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><meta name="color-scheme" content="light"><meta name="supported-color-schemes" content="light"></head><body style="margin:0;background:${BG};padding:24px 12px;font-family:${sans}">` +
    ...
    `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" bgcolor="${HEAD}"><tr><td align="center" style="padding:18px 24px 16px;background:${HEAD};border-bottom:1px solid ${LINE}"><img src="${esc(LOGO)}" alt="${esc(c.business || "")}" width="300" style="width:300px;max-width:80%;height:auto;display:block;margin:0 auto;border:0"></td></tr></table>` +
    // body

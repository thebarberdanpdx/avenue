...
const BLOCK_TAGS_RE = /(\{appointment\}|\{location\}|\{policy\}|\{cancel link\}|\{checkin link\}|\{review link\})/g;
...
export function renderEmailHtml(template, ctx) {
  ...
  const INK = "#141414", HEAD = "#0d0d0f", TEXT = "#242424", SUB = "#6b6b6b", FAINT = "#9a9a9a",
  ...
  const blocks = {
    "{appointment}": () => card(
      ...
      (addonsStr ? `<div style="color:${SUB};font-size:13px;margin-top:3px">${esc(addonsStr)}</div>` : "") +
      `<div style="color:${SUB};margin-top:8px;font-size:14.5px">with <b style="color:${INK}">${esc(c.provider)}</b></div>` +
      `<div style="color:${SUB};margin-top:2px;font-size:14.5px"><b style="color:${INK}">${esc(c.date)}</b> at <b style="color:${INK}">${esc(c.time)}</b></div>`
    ),
    "{location}": () => card(
  ...
  return `<!doctype html><html><body style="margin:0;background:${BG};padding:24px 12px;font-family:${sans}">` +
    ...
    // logo header — dark band matching the logo's background
    `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" bgcolor="${HEAD}"><tr><td align="center" style="padding:30px 24px;background:${HEAD}"><img src="${esc(LOGO)}" alt="${esc(c.business || "")}" width="230" style="width:230px;max-width:70%;height:auto;display:block;margin:0 auto;border:0"></td></tr></table>` +
    // body

...
export function renderEmailHtml(template, ctx) {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><meta name="color-scheme" content="light"><meta name="supported-color-schemes" content="light"></head><body style="margin:0;background:${BG};padding:24px 12px;font-family:${sans}">` +
    ...
    `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" bgcolor="${HEAD}"><tr><td align="center" style="padding:0;background:${HEAD};border-bottom:1px solid ${LINE}"><img src="${esc(LOGO)}" alt="${esc(c.business || "")}" width="560" style="width:100%;max-width:100%;height:auto;display:block;border:0"></td></tr></table>` +
    // body

...
| **Outage / hang drills** (Phase 3) | `outage-drill` (public menu), `booking-submit-hang` (book_public only), `booking-submit-hang-all` (**FULL hang** — proves the pre-book timeout), `manage-outage-drill`, `authed-outage-drill` (staff calendar). Each makes the backend HANG (never resolves) and asserts an honest state, not an endless spinner. |

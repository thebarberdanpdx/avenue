...
const GUARDS = [
  ...
  { needle: "addon-lib-perbarber", label: "an add-on's per-barber price/time is set ONCE in the Add-ons library (business.addOnsLibrary[i].staff), stamped onto every service's lib- group as item.staff, and addonPriceFor/addonDuration read item.staff[pid] FIRST (before the legacy per-service staff.addonPrice and the shop-wide item.price). All 3 materialize sites (AddOnsEditor, consolidateBookingLibraries, NewAppointmentForm) must carry item.staff or a booking reads a stale price. Unit-tested in tests/resolvers.test.mjs" },

...
const GUARDS = [
  ...
  { needle: "per-event-alert-scope", label: "each staff-alert EVENT routes its text/email to its own recipient group (teamScope: per-event `to` → legacy global bookingAlertScope → 'assigned'), so the owner can send e.g. new-booking texts to just the assigned barber but cancellations to all staff — 'not everyone gets every notification'. fireStaffPush passes teamScope(sa,event) as the /api/notify scope. Falls back to the old global scope with no migration, so existing shops are unchanged until a row is customized. In-app push stays shop-wide (api/push has no per-provider token link). Unit-tested (teamScope) in tests/resolvers.test.mjs. Removing it reverts to one global recipient for every event" },

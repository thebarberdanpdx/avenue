...
const GUARDS = [
  ...
  { needle: "menu-num-clearable", label: "per-barber cut-style price/time fields stay CLEARABLE — those inputs show a computed default when no override is saved, so a raw controlled input snaps the old number back the instant a digit is deleted and the barber can never retype (the exact bug Dan hit). EditableNum holds an empty draft while focused and only mirrors the derived value on blur. Removing it / reverting to value={pVal|dVal} reopens the 'can't delete the numbers' bug" },

...
const GUARDS = [
  ...
  { needle: "native-appt-delete-rail", label: "api/sync-pull saveTable HOLDS a single save that would delete an implausible fraction of a table (more than max(50, 40%) of existing rows) and returns an error instead — so a Vero-booked appointment/client can never be mass-deleted by a buggy or truncated client. A real cancel removes one/a few rows per save; a giant delete batch is always a corrupted local list. The rows stay server-side, the upserts still land, and the next mirror pull restores the client — self-heals with zero data loss. Removing it lets a catastrophic client bug wipe native bookings the iCal delete-rails don't cover" },
  { needle: "synced-past-keep", label: "a synced (iCal) appointment whose start has already PASSED is HISTORY and must NEVER be auto-cancelled/deleted on re-sync — only a bare, still-UPCOMING mirror that vanished from the feed is a real cancellation. ROOT (Dan, 2026-07-21): staff iCal feeds (Mangomint/Google/Apple) export a forward-looking window, so each day yesterday's events drop out of the feed; reconcileFeed (src/App.jsx) + reconcileFeedServer (api/calendar-run.js) treated every vanished bare mirror as cancelled, staying under the 34%/day rail, so the entire past calendar was erased one day at a time. BOTH copies keep past orphans (isPast → keptWorkedOrphans). Removing it reopens the silent past-appointment wipe" },
  ...
  { needle: "per-event-alert-scope", label: "each staff-alert EVENT routes to its own recipient group (teamScope: per-event `to` → legacy global bookingAlertScope → 'assigned'), so the owner can send e.g. new-booking alerts to just the assigned barber but cancellations to all staff — 'not everyone gets every notification'. fireStaffPush passes teamScope(sa,event) to BOTH /api/notify (text/email) and /api/push (in-app). Falls back to the old global scope with no migration. Unit-tested (teamScope) in tests/resolvers.test.mjs. Removing it reverts to one global recipient for every event" },
  { needle: "per-person-push", label: "the in-app pop-up (api/push) is delivered only to the SAME staff the text/email targets — resolved from the event's scope + the providers table, matched to each device by the auth identity save_device_token stored it under. STRICTLY read-only and FAIL-OPEN: if it can't confidently identify a device's owner as a non-recipient it keeps it, so a staff alert is never silently dropped (worst case = the old shop-wide behavior). Removing it makes every in-app pop-up go to every signed-in device again" },
  { needle: "session-cookie-fallback", label: "the signed-in CLIENT session is mirrored to a first-party cookie, not localStorage alone — the root of the recurring 'booked, added a selfie/photos, dumped to the login/welcome screen'. On iOS Private Mode, the in-app webviews an SMS/Instagram link opens in, and a WKWebView that memory-purges + reloads the page when the camera closes, localStorage.setItem silently throws / is wiped, so the tiny identity record + full record left NO durable copy and every restore path found no session → login. The cookie is written alongside the localStorage tiny record and read as a fallback at all three restore sites (mount, welcome-guard, confirmation-exit); explicit sign-out clears it too. Commits #503/#517/#518 only shrank the localStorage payload and never removed the sole-dependency on it. Removing this reopens the dumped-to-login class on exactly the browsers real clients use" },
  { needle: "session-shop-stable", label: "the version auto-update hardReload PRESERVES every existing query param (above all ?shop=) across the cache-bust reload — resolveShopId reads ?shop= first, so dropping it fell back to the default shop, which changed the per-shop client-session key (vero_client_<shop>) and dumped a just-booked client to login (and reloaded the WRONG shop's data). Only the cache-bust key `u` changes. Reverting to origin+path+?u= reopens the shop-flip-on-reload logout" },
  ...
  { needle: "selfie-reward-ledger", label: "the $5 'add a profile selfie' credit is a ONE-TIME-per-client reward gated by a durable per-client `selfieRewarded` flag — set the moment it's granted (applySelfie), checked before every offer (ConfirmationScreen selfieEligible), booking-commit discount, and RPC write. It replaced the old photo-only gate, which was leaky in practice (verified live 2026-07-23: 4 clients got the credit but only 1 had a saved photo → the other 3 stayed 're-eligible' under `!photo`). The selfie also LOCKS on add (no in-session Remove button) so the photo can't be cleared to re-qualify. Server backstop (un-bypassable via a direct RPC replay) + retroactive backfill: db/selfie-reward-ledger-2026-07-23.sql. Removing it reopens $5-credit farming on repeat bookings" },
];
try {
  const app = readFileSync(join(ROOT, "src/App.jsx"), "utf8");

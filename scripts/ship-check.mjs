...
const GUARDS = [
  ...
  { needle: "cut-base-adds-model", label: "the simple cut-style editor stores as base + a shared '+add' per style but the pricing ENGINE still reads per-barber ABSOLUTE totals (staff.choicePrice/choiceDur, setsPrice) — recomputeChoiceAbsolutes writes those on SAVE (price shared = form.price + option add; time per-barber = staff.duration + option add) and deriveCutBaseAndAdds turns them back into base+adds on OPEN. A no-edit open→save is price-exact (verified on prod). Removing recomputeChoiceAbsolutes from save() stops writing the absolutes the booking/checkout/reports engine reads → cut prices break" },
  { needle: "recomputeChoiceAbsolutes(", label: "save() materializes the cut-style base+adds into per-barber absolute totals via recomputeChoiceAbsolutes — the booking engine reads those absolutes, so this call is what keeps a saved cut priced correctly" },
